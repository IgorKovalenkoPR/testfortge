...
class TestCase:
    ...
    # Walkthrough integration fields (Sprint 5 / PR-2 DB columns,
    # surfaced to the dataclass in the follow-up that ships the
    # /test-cases editor UI). ``url_pattern`` is an fnmatch glob the
    # walkthrough runner uses to decide whether to fire this TC on a
    # visited page; ``trigger`` is "manual" (default — never fires
    # from walkthrough), "walkthrough_url_match" (fires when pattern
    # matches), or "always" (fires on every page). Keep defaults
