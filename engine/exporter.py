...
def export_test_plan_markdown(plan) -> str:
    """Render a :class:`TestPlan` to Markdown (Sprint 4 task 4.3).

    Produces a TOC + 13 H2 sections (numbered ``N. Title``) plus all
    subsections (``N.M Title`` as H3) and tables. The output round-trips
    through any Markdown renderer that supports GFM tables.
    """
    lines: list[str] = []
    a = lines.append

    a(f"# Test Plan — {plan.project_name}")
    a(f"\n**Version:** {plan.version}  ")
    a(f"**Date:** {plan.date}\n")

    # Table of contents — one entry per top-level section.
    a("## Table of Contents\n")
    for section in plan.sections:
        a(f"{section.number}. [{section.title}](#section-{section.number})")
    a("")

    for section in plan.sections:
        a(f"\n## <a id=\"section-{section.number}\"></a>"
        ...
        for table in section.tables or []:
            ...
            if title:
                a(f"### {title}\n")
            if headers:
                a("| " + " | ".join(str(h) for h in headers) + " |")
                a("|" + "|".join("---" for _ in headers) + "|")
                for row in rows:
                    # Markdown table cells can't contain raw newlines —
                    # collapse to ``<br>`` so multi-line responsibility
                    # blurbs still survive the export.
                    safe_cells = [
                        str(c).replace("\n", "<br>").replace("|", "\\|")
                        for c in row
                    ]
