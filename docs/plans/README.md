...
| [adr/0002-artifact-storage.md](adr/0002-artifact-storage.md) | ADR · **Accepted 2026-08-08** | Artefact storage for R3: local disk + one S3-compatible adapter, keyed per org/project, configured per organisation over an instance default. Approved including §4.1 (Azure deferred); §4.5's package choice was reversed by measurement at implementation time — `minio`, not `boto3`. E8.2 shipped the same day | — |
